...
async function getAllFruits() {
    ...
    let fruits: string[] = await fetchRestEndpoint('http://localhost:3000/api/fruits', 'GET');
    document.getElementById('result').innerText = fruits.join(', ');
}

async function getFirstFruit() {
    try {
        let fruit: string = await fetchRestEndpoint('http://localhost:3000/api/fruits/0', 'GET');
...
async function addFruit() {
    ...
    let fruit: string = await fetchRestEndpoint('http://localhost:3000/api/fruits', 'POST', data);
    document.getElementById('result').innerText = fruit;
}

async function deleteAllFruits() {
    await fetchRestEndpoint('http://localhost:3000/api/fruits', 'DELETE');
    document.getElementById('result').innerText = 'deleted all fruits'; 
}

async function deleteFirstFruit() {
    try {
        await fetchRestEndpoint('http://localhost:3000/api/fruits/0', 'DELETE');
